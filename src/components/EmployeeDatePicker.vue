<template>
  <div class="chl-datepicker chl-datepicker-employee">
    <vc-date-picker
      ref="datepicker"
      v-model="date"
      mode="date"
...
export default {
  ...
  computed: {
    ...mapState(['days', 'selectedId', 'selectedCalendarId']),
    getInputDate() {
      if (this.selectedId) {
    ...
    attributes() {
      return this.days.map((date, index) => ({
        ...
        highlight: {
            color: 'black',
            fillMode: 'outline',
            contentClass: 'booked' // class does not stay on selected element in DatePicker
          },
        dates: date,
      }));
    }
